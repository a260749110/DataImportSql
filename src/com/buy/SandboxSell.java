...
public class SandboxSell {
	...
	public void testCanSell(CBuySellPo po, List<LycjssFlagData> datas) {
		...
		for (int i = 0; i < datas.size(); i++) {
			...
			if (i == datas.size() - 1) {
				...
				if ((po.getBuyMoney() * (1 + Config.win_per) < data.getClose() * 1.101)
						...
						&& (i == datas.size() - 1)) {
					// sb.append("id:" + po.getId() + " 在" +
					// dateHelper.dateFormat.format(data.getDate()) +
					// "冲击涨停不需要挂牌卖出"
					// + "，冲击价格：" + po.getBuyMoney() * (1 +
					// Config.win_per)).append("\r\n");
		...
		List<CDataBasePo> pos = AppContextUtil.instance.getCDataBaseDao().findAfByIdAndDate(po.getId(),
				dateHelper.dateFormat.format(po.getBuyDate()));
		if (pos.size() >= Config.max_keep) {
			System.err.println("id:" + po.getId() + " 下个交易日挂（超出：" + Config.max_keep + "天）："
					+ df.format(po.getBuyMoney() * (1 + Config.win_per)) + "卖出-----------");
		}
		if (sb.length() > 0)
